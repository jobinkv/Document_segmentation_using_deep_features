function [code, codeLoc] = get_dcnn_features_modified(net,im,varargin)
% load('dcnnInputs.mat')
...
%                assign = zeros(opts.encoder.numWords, numel(words), 'single') ;
 ...
 %         tmp{end+1} = z;
   ...
   %   end
      ...
      code{k} =z;% cat(1, tmp{:}) %/ (opts.numSpatialSubdivisions(1) * opts.numSpatialSubdivisions(2)) ;
    end
%    code{k} = cat(2, code{k}{:}) ;
 % end
%  if nargout == 1
